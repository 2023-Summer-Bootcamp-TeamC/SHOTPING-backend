...
def save_image(id, prediction):
    ...
    engine = create_engine(
        "mysql+pymysql://user:1234@shotping-mysql.co7pnzdwwqty.ap-southeast-2.rds.amazonaws.com/shotping"
    )
    ...
    # 이미 있는 테이블에 연결
    data = Table("Data", metadata, autoload_with=engine)

    try:
        # 데이터 수정
        if session.query(data).filter_by(id=id).first():
            update_stmt = (
                data.update().where(data.c.id == id).values(ai_predict=prediction)
            )
